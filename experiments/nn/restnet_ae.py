...
class ResNetAutoEncoder(nn.Module):
    ...
    def __init__(self, configs, bottleneck, skip_connection=False):
    ...
    def forward(self, x_):
        x = self.encoder(x_)
        x = self.decoder(x, x_ if self.skip_connection else None)
        return x
...
class ResNetEncoder(nn.Module):
    ...
    def __init__(self, configs, bottleneck=False):
        ...
        if bottleneck:
            ...
            self.conv2 = EncoderBottleneckBlock(in_channels=64, hidden_channels=64, up_channels=256, layers=configs[0],
