...
from experiments.nn.vision_cortex_variable import VisionCortex, VisionCortexSize
...
config = {
    ...
    'config': {
        ...
        'img_size': (112, 112),
        '{data_loader}': lambda: loader('train', inputs=['c:0'], outputs=['c:1']),
        # 'train_ic_transform': transform(),

        # network
        '{model}': lambda: VisionCortex(
            size=VisionCortexSize(1, 1, 64),
            train_depths=(0, 1)
