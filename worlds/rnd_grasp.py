...
class PickAndPlaceBehaviour:
    ...
    def __init__(self, injector: Injector, config: ConfigBlock):
        ...
        self.config = config
        self.memory = Memory('world_blocks2', self.body, self.config, skip_right_sensor=False)
        ...
        self.START_POS_UP = [0.3, -0.5, 0.21]
    ...
    def on_start(self):
        ...
        for i in range(10):
            move_arm([
                self.d[0][0] + (self.d[0][1] - self.d[0][0]) * random(),
                self.d[1][0] + (self.d[1][1] - self.d[1][0]) * random(),
                self.d[2][0] + (self.d[2][1] - self.d[2][0]) * random()
            ])
            move_gripper(0.26 + (0.5 - 0.26) * random())
    ...
    run_all(launch_world, {
        ...
        'pick_blocks': lambda: flatten([
            [
                ...
                for i in range(randint(0, max_tower_height))
            ]
